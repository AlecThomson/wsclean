  ...
  if (!_settings.facetSolutionFile.empty()) {
    ...
    if (_settings.facetSolutionTables.size() == 1) {
      ...
      if (!_h5SolTabs.first->HasAxis("pol") &&
        ...
        if (_h5SolTabs.second->GetAxis("pol").size == 1) {
          _correctType = JonesParameters::CorrectType::SCALARGAIN;
        } else {
          throw std::runtime_error(
              "The phase solution table should have length one!");
        }
      } else if (_h5SolTabs.first->HasAxis("pol") &&
                 !_h5SolTabs.second->HasAxis("pol")) {
        if (_h5SolTabs.first->GetAxis("pol").size == 1) {
          _correctType = JonesParameters::CorrectType::SCALARGAIN;
        } else {
          throw std::runtime_error(
              "The amplitude solution table should have length one!");
        }
      } else if (!_h5SolTabs.first->HasAxis("pol") &&
                 !_h5SolTabs.second->HasAxis("pol")) {
        _correctType = JonesParameters::CorrectType::SCALARGAIN;
