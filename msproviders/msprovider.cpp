  ...
  if (polSource == aocommon::Polarization::Instrumental) {
    for (size_t chp = 0; chp != selectedChannelCount * polsDest.size(); ++chp) {
      if (std::isfinite(source[chp].real())) {
        // *dataIter = source[chp];
        // assignToMS(dataIter, source[chp]);
    ...
    for (size_t ch = 0; ch != selectedChannelCount; ++ch) {
      if (std::isfinite(source[ch].real())) {
        ...
        // assignToMS(dataIter + polIndex, source[ch]);
      ...
      case aocommon::Polarization::StokesI: {
        ...
        for (size_t ch = 0; ch != selectedChannelCount; ++ch) {
          if (std::isfinite(source[ch].real())) {
            ...
            // assignToMS(dataIter + polIndexA, source[ch]);  // XX = I (or rr =
            // I) assignToMS(dataIter + polIndexB, source[ch]);  // YY = I (or
            // ll = I)
        ...
        if (hasXX && hasYY) {
          ...
          for (size_t ch = 0; ch != selectedChannelCount; ++ch) {
            if (std::isfinite(source[ch].real())) {
              ...
              // assignToMS(dataIter + polIndexA,
              //            stokesI + source[ch]);  // XX = I + Q
              // assignToMS(dataIter + polIndexB,
        ...
        if (hasXY && hasYX) {
          ...
          for (size_t ch = 0; ch != selectedChannelCount; ++ch) {
            if (std::isfinite(source[ch].real())) {
              ...
              // *(dataIter + polIndexA) = stokesQ + iTimesStokesU;  // rl = Q +
              // iU
              // *(dataIter + polIndexB) = stokesQ - iTimesStokesU;  // lr = Q -
              // iU
              // assignToMS(dataIter + polIndexA,
...
std::vector<aocommon::PolarizationEnum> MSProvider::GetMSPolarizations(
  ...
  casacore::ROArrayColumn<int> corrTypeColumn(
