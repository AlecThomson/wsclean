...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename TBuf> class Wgridder
  {
  private:
    constexpr static int log2tile=is_same<Tacc,float>::value ? 5 : 4;
    bool gridding;
    TimerHierarchy timers;
    const cmav<complex<Tms>,2, TBuf> &ms_in;
...
//            if (mask(irow,ichan) && (wgt(irow, ichan)!=0) && (norm(ms_in(irow,ichan)!=0)))
  ...
  public:
    Wgridder(const cmav<double,2> &uvw, const cmav<double,1> &freq,
           const cmav<complex<Tms>,2, TBuf> &ms_in_, const vmav<complex<Tms>,2> &ms_out_,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename TBuf> void ms2dirty(const cmav<double,2> &uvw,
  const cmav<double,1> &freq, const cmav<complex<Tms>,2, TBuf> &ms,
  ...
  Wgridder<Tcalc, Tacc, Tms, Timg, TBuf> par(uvw, freq, ms, ms_out, dirty_in, dirty, wgt, mask, pixsize_x,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg> void dirty2ms(const cmav<double,2> &uvw,
  ...
  auto wgt(wgt_.size()!=0 ? wgt_ : wgt_.build_uniform(ms.shape(), 1.));
  auto mask(mask_.size()!=0 ? mask_ : mask_.build_uniform(ms.shape(), 1));
  Wgridder<Tcalc, Tacc, Tms, Timg, ducc0::detail_mav::cmembuf<complex<Tms>>> par(uvw, freq, ms_in, ms, dirty, dirty_out, wgt, mask, pixsize_x,
    pixsize_y, epsilon, do_wgridding, nthreads, verbosity, negate_v,
    divide_by_n, sigma_min, sigma_max, center_x, center_y, allow_nshift);
  }
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename TBuf> void ms2dirty_faceted(size_t nfx, size_t nfy, const cmav<double,2> &uvw, const cmav<double,1> &freq, const cmav<complex<Tms>,2, TBuf> &ms,
  const cmav<Tms,2> &wgt_, const cmav<uint8_t,2> &mask_, double pixsize_x, double pixsize_y, double epsilon,
  bool do_wgridding, size_t nthreads, const vmav<Timg,2> &dirty, size_t verbosity,
  bool negate_v=false, bool divide_by_n=true, double sigma_min=1.1,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename TBuf> void ms2dirty_tuning(const cmav<double,2> &uvw,
  const cmav<double,1> &freq, const cmav<complex<Tms>,2, TBuf> &ms,
  const cmav<Tms,2> &wgt_, const cmav<uint8_t,2> &mask_, double pixsize_x, double pixsize_y, double epsilon,
  bool do_wgridding, size_t nthreads, const vmav<Timg,2> &dirty, size_t verbosity,
  bool negate_v=false, bool divide_by_n=true, double sigma_min=1.1,
