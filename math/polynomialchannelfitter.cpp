...
void PolynomialChannelFitter::Fit(std::vector<double>& terms, size_t nTerms) {
  ...
  gsl_multifit_linear_workspace* work =
      gsl_multifit_linear_alloc(nPoints, nTerms);
  ...
  for (size_t i = 0; i != nPoints; ++i) {
    ...
    const double nuBegin = _channels[chIndex].first,
                 nuEnd = _channels[chIndex].second;
    ...
    for (size_t j = 1; j != nTerms; ++j) {
      ...
      double val =
          (nuEndTerm - nuBeginTerm) / (double(j + 1) * (nuEnd - nuBegin));
