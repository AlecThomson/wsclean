...
BOOST_AUTO_TEST_CASE(store_and_load_facet) {
  ...
  FitsWriter writer;
  writer.SetImageDimensions(image_width, image_height, dl, dl);
  ...
  for (size_t i = 0; i < num_facets; ++i) {
    ...
    facets[i].CalculatePixelPositions(
        writer.RA(), writer.Dec(), dl, dl, writer.Width(), writer.Height(),
        writer.PhaseCentreDL(), writer.PhaseCentreDM(), false);
  ...
  CachedImageSet cSet;
  cSet.Initialize(writer, 2, 1, prefix, facets.size());
  ...
  for (auto& polarization : polarizations) {
    for (size_t facet_idx = 0; facet_idx < facets.size(); ++facet_idx) {
      cSet.StoreFacet(facets_data[facet_idx].data(), polarization, 1, false,
                      facet_idx, facets[facet_idx].GetBoundingBox().Width(),
