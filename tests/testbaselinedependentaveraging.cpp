...
BOOST_AUTO_TEST_CASE(noAveraging) {
  ...
  if (boost::filesystem::exists(filename)) {
    ...
    AveragingMSRowProvider avgProvider(1e-8, filename, selection, dataDescIds,
                                       0, "DATA", false);
    DirectMSRowProvider directProvider(filename, selection, dataDescIds, "DATA",
                                       false);
    ...
    while (!avgProvider.AtEnd() && !directProvider.AtEnd() &&
      ...
      directProvider.ReadData(dataArrayDirect, flagArrayDirect, weightsDirect,
                              uD, vD, wD, descD, a1D, a2D, field, timeD);

      for (size_t p = 0; p != 4; ++p) {
        std::complex<float> valA = dataArrayAvg.data()[p];
        std::complex<float> valD = dataArrayDirect.data()[p];
        bool isAFinite =
            std::isfinite(valA.real()) && std::isfinite(valA.imag());
        bool isDFinite =
            std::isfinite(valD.real()) && std::isfinite(valD.imag());
