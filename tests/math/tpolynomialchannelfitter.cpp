...
BOOST_AUTO_TEST_CASE(fit_squared_function) {
  ...
  fitter.AddDataPoint(
      1, 19.0 / 3.0);  // int_{x=2}^2 x^2 / 1 = 1/3 (3^3 - 2^3) = 19/3
  fitter.AddDataPoint(
...
BOOST_AUTO_TEST_CASE(fit_irregular_channels) {
  ...
  fitter.AddDataPoint(
      1, 19.0 / 3.0);  // int_{x=2}^3 x^2 / 1 = 1/3 (3^3 - 2^3) = 19/3
  fitter.AddDataPoint(
...
BOOST_AUTO_TEST_CASE(fit_squared_missing_bandwidth) {
  ...
  fitter.AddDataPoint(
      1, 728.0 / 6.0);  // int_{x=10}^12 x^2 / 1 = 1/2 (12^3 - 10^3) = 728/6
  fitter.AddDataPoint(
...
double integrate(double xa, double xb, double a, double b, double c, double d) {
  return ((a * xb + b * xb * xb / 2.0 + c * xb * xb * xb / 3.0 +
           d * xb * xb * xb * xb / 4.0) -
          (a * xa + b * xa * xa / 2.0 + c * xa * xa * xa / 3.0 +
           d * xa * xa * xa * xa / 4.0)) /
         (xb - xa);
}
