...
MultiBandData::MultiBandData(const MultiBandData& source, size_t startChannel,
  ...
  for (size_t spw = 0; spw != source.BandCount(); ++spw) {
    ...
    if (startChannel > bandEndChannel)
      throw std::runtime_error("Invalid band selection: MultiBandData constructed with startChannel=" + std::to_string(startChannel) + ", nr of channels is " + std::to_string(bandEndChannel));
    _bandData[spw] =
